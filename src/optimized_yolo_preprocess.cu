    ...
    YoloPreprocessCircleMask circle_mask)
{
    ...
    if (dst_x >= pad_left && dst_x < (dst_width - pad_left) && 
        ...
        if constexpr (ApplyCircleMask) {
            const float dx = src_x_f - circle_mask.center_x;
            const float dy = src_y_f - circle_mask.center_y;
            sample_source = dx * dx + dy * dy <= circle_mask.radius_squared;
        }
        if (sample_source) {
    ...
    YoloPreprocessCircleMask circle_mask)
{
    ...
    const float default_value = ApplyCircleMask
        ? circle_mask.outside_tensor_value * 255.0f
        : 114.0f;
    float r_val = default_value, g_val = default_value, b_val = default_value;
...
template <bool ApplyCircleMask>
void launch_optimized_yolo_preprocess_impl(
    ...
    cudaStream_t stream)
{
    ...
    if (is_color) {
        bayer_to_yolo_optimized<ApplyCircleMask><<<grid, block, 0, stream>>>(
            d_src, d_dst_planar, src_width, src_height,
            dst_width, dst_height, scale_x, scale_y,
            pad_left, pad_top, 0, circle_mask); // Assuming RGGB for now.
    } else {
        mono_to_yolo_optimized<ApplyCircleMask><<<grid, block, 0, stream>>>(
            d_src, d_dst_planar, src_width, src_height,
            dst_width, dst_height, scale_x, scale_y,
            pad_left, pad_top, circle_mask);
    }
}

}  // namespace

// Two compiled preprocessing specializations feed the same TensorRT input.
extern "C" {
void launch_optimized_yolo_preprocess(
    ...
    cudaStream_t stream)
{
    const YoloPreprocessCircleMask unused_mask{};
    launch_optimized_yolo_preprocess_impl<false>(
        d_src, d_dst_planar,
        src_width, src_height,
        dst_width, dst_height,
        is_color, unused_mask, stream);
}
...
void launch_optimized_yolo_preprocess_circle_masked(
    ...
    cudaStream_t stream)
{
    launch_optimized_yolo_preprocess_impl<true>(
        d_src, d_dst_planar,
        src_width, src_height,
        dst_width, dst_height,
