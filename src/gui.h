...
inline void setup_texture(GL_Texture& tex, int width, int height) {
    ...
    // Map the resource to get the pointer
    ...
    // Unmap the resource immediately after getting the pointer.
...
inline void clear_upload_and_cleanup(GL_Texture& tex, int width, int height) {
    ...
    // Unmap the resource before unregistering and cleaning up
    if (tex.cuda_resource) {
      unmap_cuda_resource(&tex.cuda_resource); // Match the map call
