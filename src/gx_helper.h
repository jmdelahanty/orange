...
inline void gx_glfw_error_callback(int error, const char *description)
{
    fprintf(stderr, "Glfw Error %d: %s\n", error, description);
}

inline void gx_glew_error_callback(GLenum glew_error)
{
...
inline GLFWwindow *gx_glfw_init_render_target(u32 marjor_version, u32 minor_version, u32 width, u32 height, const char *title, char *glsl_version)
{
    // Setup window
...
inline void gx_delete_buffer(GLuint *texture) 
{
	glDeleteBuffers(1, texture);
}

inline void create_texture(GLuint *texture, int image_width, int image_height)
{
...
inline void create_pbo(GLuint *pbo, int image_width, int img_height)
{
    glGenBuffers(1, pbo);
...
inline void upload_image_pbo_to_texture(int image_width, int image_height)
{
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, image_width, image_height, GL_RGBA, GL_UNSIGNED_BYTE, 0);
}


inline void bind_pbo(GLuint *pbo)
{
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, *pbo);
}

inline void unbind_pbo()
{
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
}

inline void register_pbo_to_cuda(GLuint *pbo, cudaGraphicsResource_t *cuda_resource)
{
    cudaGraphicsGLRegisterBuffer(cuda_resource, *pbo, cudaGraphicsRegisterFlagsNone);
}

inline void cuda_unregister_pbo(cudaGraphicsResource_t cuda_resource)
{
    cudaGraphicsUnregisterResource(cuda_resource);
}

inline void map_cuda_resource(cudaGraphicsResource_t *cuda_resource, cudaStream_t stream)
{
    cudaGraphicsMapResources(1, cuda_resource, stream);
}

inline void cuda_pointer_from_resource(unsigned char **cuda_buffer_p, size_t *size_p, cudaGraphicsResource_t *cuda_resource)
{
    cudaGraphicsResourceGetMappedPointer((void **)cuda_buffer_p, size_p, *cuda_resource);
}

inline void unmap_cuda_resource(cudaGraphicsResource_t *cuda_resource)
{
    cudaGraphicsUnmapResources(1, cuda_resource);
}


inline void render_initialize_target(gx_context *window)
{
...
inline void render_a_frame(gx_context *window)
{
...
inline void gx_cleanup(gx_context *window)
{
