...
| Recording geometry contract | `<recording_folder>/recording_geometry_contract.json` | Required for current GUI/headless recordings | Pre-arm resolves participating camera geometry; mask-off recordings may explicitly carry no selected physical registration |
| Recording geometry asset bundle | `<recording_folder>/recording_geometry_assets/manifest.json` plus scoped files | Required when a selected physical registration or live mask depends on copied evidence | Exact active pointer, observation, manifest, compact mask exports, and optional review images are copied with SHA-256; enabled live masking requires a complete bundle |
...
Newly written Orange MP4 finalization sidecars use
`orange.video_container_finalization` schema version 2. Its `packet_writes`
proof distinguishes queue acceptance from successful FFmpeg mux calls. A
terminal `complete` claim requires zero rejected submissions and write
failures, equal accepted/attempted/written packet counts, no latched writer
error, and an attempted, successful muxer flush. The same document separately
proves trailer, close, and typed playback-intent patch outcomes. Historical
Orange and current Citrus schema-version-1 sidecars remain valid inputs to the
validator. See
[`orange_video_container_finalization.schema.json`](schemas/orange_video_container_finalization.schema.json)
and the detailed semantics in
[`recording_metadata.md`](recording_metadata.md#embedded-playback-intent-and-container-finalization).
