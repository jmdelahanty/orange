...
| Observation binding pre-arm decision (Orange-only sessions: `<recording_folder>/recording_observation_pre_arm_decision.json`, so `recording_observation_bindings/` stays absent for Citrus transfer-v2) | `<recording_folder>/recording_observation_bindings/pre_arm_decision.json` | Required for newly started recordings under all three binding modes | Create-once before recorder/acquisition activation; records arm policy and controlled unbound reason |
