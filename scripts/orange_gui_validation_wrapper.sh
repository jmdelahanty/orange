...
validate_env_item() {
  ...
  case "$key" in
    ...
    ORANGE_GUI_GUIDED_CAPTURE_SWEEP_FOREGROUND_GRAYS_U8)
      ...
      ;;
    ORANGE_YOLO_PERF_LOG|ORANGE_CROP_COPY_TIMING|ORANGE_CROP_STAGE_SOURCE|ORANGE_ANALYTICS_EARLY_OWNED_FRAME|ORANGE_YOLO_DETACH_INPUT|ORANGE_YOLO_READY_EVENT_FASTPATH|ORANGE_RECORDING_DETECT_PRIORITY|ORANGE_ACQ_RING_RELEASE_LOG|ORANGE_ACQ_CADENCE_PROBE_ALL|ORANGE_GUI_SKIP_PBO_UPLOAD|ORANGE_DISPLAY_PREVIEW_DISABLE|ORANGE_DISPLAY_SOURCE_DOWNSAMPLE|ORANGE_HOST_STALL_MONITOR|ORANGE_GUI_POSE_OVERLAY|ORANGE_GUI_SHOW_SPEED_GRAPHS|ORANGE_GUI_AUTORUN|ORANGE_GUI_AUTORUN_EXIT_AFTER_FINALIZE|ORANGE_GUI_AUTORUN_HIDE_CROP_PREVIEW|ORANGE_GUI_AUTORUN_ENABLE_STREAM|ORANGE_GUI_AUTORUN_ENABLE_RECORD|ORANGE_GUI_AUTORUN_ENABLE_YOLO|ORANGE_GUI_AUTORUN_ENABLE_CROP|ORANGE_GUI_AUTORUN_ENABLE_POSE|ORANGE_GUI_AUTORUN_START_RECORDING|ORANGE_GUI_AUTORUN_STOP_STREAMING_AFTER_WARMUP|ORANGE_GUI_AUTORUN_KEEP_STREAMING_AFTER_FINALIZE|ORANGE_GUI_GUIDED_CAPTURE_AUTORUN|ORANGE_GUI_GUIDED_CAPTURE_SAVE|ORANGE_GUI_GUIDED_CAPTURE_EXIT_AFTER_COMPLETION|ORANGE_GUI_GUIDED_CAPTURE_APPLY_CALIBRATION_PREFLIGHT|ORANGE_GUI_GUIDED_CAPTURE_INCLUDE_ARENA_OUTLINE_REFERENCE|ORANGE_GUI_GUIDED_CAPTURE_FIT_HOMOGRAPHIES|ORANGE_GUI_PROJECTED_SURFACE_SCALE_TARGETS_READY|ORANGE_GUI_ACCEPT_PROJECTED_SURFACE_SCALES_ARMED|ORANGE_GUI_ARENA_CENTERING_AUTORUN|ORANGE_GUI_ARENA_CENTERING_BOOTSTRAP_CENTERS_ONLY|ORANGE_GUI_ARENA_CENTERING_APPLY_CALIBRATION_PREFLIGHT|ORANGE_GUI_ARENA_CENTERING_SAVE_CAPTURES|ORANGE_GUI_ARENA_CENTERING_SAVE_VERIFIED_CENTERS_ARMED|ORANGE_GUI_ARENA_CENTERING_RESIZE_ARENAS|ORANGE_GUI_ARENA_CENTERING_SAVE_VERIFIED_LAYOUT_ARMED|ORANGE_GUI_ARENA_CENTERING_FIT_HOMOGRAPHIES|ORANGE_GUI_ARENA_CENTERING_ACCEPT_HOMOGRAPHIES_ARMED|ORANGE_GUI_ARENA_CENTERING_EXIT_AFTER_COMPLETION|ORANGE_GUI_ARENA_CENTERING_REQUIRE_STABILITY_CAPTURE|ORANGE_CROP_EXTERNAL_REQUIRE_SEPARATE_GPU|ORANGE_GUI_EXTERNAL_RECORDER_CONTRACT|ORANGE_CROP_PREVIEW_DISABLE|ORANGE_SHAMAN_V2_LIVE_STATE|ORANGE_LOCAL_CONTROL_DISABLE|ORANGE_GUI_LOCAL_CONTROL_DISABLE|ORANGE_LOCAL_CONTROL_ENABLE_RECORDING_START|ORANGE_GUI_LOCAL_CONTROL_ENABLE_RECORDING_START|ORANGE_LOCAL_CONTROL_ENABLE_RECORDING_STOP|ORANGE_GUI_LOCAL_CONTROL_ENABLE_RECORDING_STOP|ORANGE_LOCAL_CONTROL_ENABLE_CITRUS_STOP|ORANGE_GUI_LOCAL_CONTROL_ENABLE_CITRUS_STOP|ORANGE_LOCAL_CONTROL_EXIT_AFTER_FINALIZE|ORANGE_GUI_LOCAL_CONTROL_EXIT_AFTER_FINALIZE)
      is_bool "$value" || {
...
# Host stall monitor (scripts/host_stall_monitor.py from the same source tree
# as the Orange binary): a 1 ms heartbeat on a housekeeping core, 1 Hz kernel
# counters and the SMI counter, running as root beside Orange. Default on;
# ORANGE_HOST_STALL_MONITOR=0 disables it. Output lands in
# /home/jeremy/orange_data/diagnostics/host_stall/<utc stamp>_{heartbeat,counters}.csv
# and _summary.json, chowned to the invoking user; Orange records the prefix
# in recording_snapshot.json (session.host_stall_monitor_prefix).
stall_monitor_pid=""
stall_monitor_prefix=""
if [[ "${ORANGE_HOST_STALL_MONITOR:-1}" == "1" && "$DRY_RUN" != "1" ]]; then
  stall_monitor_script="$(realpath -m "$(dirname "$ORANGE_BIN")/../../scripts/host_stall_monitor.py")"
  case "$ORANGE_BIN" in
    "$DEFAULT_ORANGE_BIN") stall_monitor_script="$DEFAULT_ORANGE_ROOT/scripts/host_stall_monitor.py" ;;
  esac
  if [[ -f "$stall_monitor_script" ]]; then
    ...
    echo "[sudo-wrapper] host_stall_monitor pid=$stall_monitor_pid prefix=$stall_monitor_prefix"
  else
    echo "[sudo-wrapper] host_stall_monitor script not found at $stall_monitor_script; monitor disabled" >&2
  fi
fi
stop_stall_monitor() {
  if [[ -n "$stall_monitor_pid" ]]; then
    touch "${stall_monitor_prefix}.stop" 2>/dev/null || true
    for _ in $(seq 1 40); do
      kill -0 "$stall_monitor_pid" 2>/dev/null || break
      sleep 0.25
    done
    kill -TERM "$stall_monitor_pid" 2>/dev/null || true
    wait "$stall_monitor_pid" 2>/dev/null || true
    if [[ -n "${SUDO_UID:-}" && -n "${SUDO_GID:-}" ]]; then
      chown -- "${SUDO_UID}:${SUDO_GID}" "${stall_monitor_prefix}"_* "${stall_monitor_prefix}.log" 2>/dev/null || true
...
trap 'stop_parent_guard; stop_stall_monitor' EXIT
