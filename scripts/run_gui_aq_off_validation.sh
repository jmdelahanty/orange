...
# Detect-latency levers (docs/detect_latency_review_2026_09_03.md). Defaults
# match the code defaults; override in the environment for an A/B run.
YOLO_SYNC_EVENT="${ORANGE_YOLO_SYNC_EVENT:-1}"
PTP_LATCH_AFTER_FANOUT="${ORANGE_PTP_LATCH_AFTER_FANOUT:-1}"
EXTERNAL_RECORDER_DIRECT_INPUT="${ORANGE_EXTERNAL_RECORDER_DIRECT_INPUT:-0}"
EXTERNAL_RECORDER_DETECT_PRIORITY="${ORANGE_EXTERNAL_RECORDER_DETECT_PRIORITY:-1}"
...
ENV_ARGS=(
  ...
  "ORANGE_YOLO_DETACH_INPUT=${YOLO_DETACH_INPUT}"
  "ORANGE_YOLO_SYNC_EVENT=${YOLO_SYNC_EVENT}"
  "ORANGE_PTP_LATCH_AFTER_FANOUT=${PTP_LATCH_AFTER_FANOUT}"
  "ORANGE_EXTERNAL_RECORDER_DIRECT_INPUT=${EXTERNAL_RECORDER_DIRECT_INPUT}"
  "ORANGE_EXTERNAL_RECORDER_DETECT_PRIORITY=${EXTERNAL_RECORDER_DETECT_PRIORITY}"
...
if [[ -n "${ORANGE_SHAMAN_V2_LIVE_STATE:-}" ]]; then
  ENV_ARGS+=("ORANGE_SHAMAN_V2_LIVE_STATE=${ORANGE_SHAMAN_V2_LIVE_STATE}")
fi
while IFS= read -r var_name; do
  [[ -n "${var_name}" ]] || continue
  case "${var_name}" in
...
if [[ -n "${ORANGE_GUI_AUTORUN_KEEP_STREAMING_AFTER_FINALIZE:-}" ]]; then
  ENV_ARGS+=("ORANGE_GUI_AUTORUN_KEEP_STREAMING_AFTER_FINALIZE=${ORANGE_GUI_AUTORUN_KEEP_STREAMING_AFTER_FINALIZE}")
fi
if [[ -n "${ORANGE_GUI_POSE_OVERLAY:-}" ]]; then
  ENV_ARGS+=("ORANGE_GUI_POSE_OVERLAY=${ORANGE_GUI_POSE_OVERLAY}")
fi
if [[ -n "${ORANGE_HOST_STALL_MONITOR:-}" ]]; then
  ENV_ARGS+=("ORANGE_HOST_STALL_MONITOR=${ORANGE_HOST_STALL_MONITOR}")
fi
if [[ -n "${ORANGE_DISPLAY_SOURCE_DOWNSAMPLE:-}" ]]; then
  ENV_ARGS+=("ORANGE_DISPLAY_SOURCE_DOWNSAMPLE=${ORANGE_DISPLAY_SOURCE_DOWNSAMPLE}")
fi
if [[ -n "${APP_CONFIG_ENV_KEY}" ]]; then
